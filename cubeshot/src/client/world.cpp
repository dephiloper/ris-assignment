...
void World::setup() {
    ...
    unsigned int textureId = loadTexture(vao, ASSETS_DIR + "cube0_texture.png", false);

    blueprints.insert(std::pair(CUBE_0, std::pair(vao, textureId)));
    textureId = loadTexture(vao, ASSETS_DIR + "cube1_texture.jpg", false);
...
void World::render() {
    ...
    for (int i = -10; i < 10; i++) {
        for (int j = -10; j < 10; j++) {
            ...
            model = glm::translate(model, glm::vec3(j, -1, i));
...
unsigned int World::loadTexture(unsigned int vao, const std::string& texturePath, bool alphaChannel) {
