...
ClientNetManager::ClientNetManager(const std::string &ipAddress, const int port) : NetManager(zmq::socket_type::dealer,
                                                                                              ipAddress, port) {}
...
void ClientNetManager::receiveData() {
    while (isRunning.load()) {
        ...
        switch (cmd) {
            ...
            case LOGOUT:
            case MOVE: {
                std::cout << "Error: the command can't get processed by the client." << std::endl;
                break;
            }
        }
